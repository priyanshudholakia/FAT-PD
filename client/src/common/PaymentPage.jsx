...
export default function PaymentPage() {
  ...
  const flight = {
    ...defaultFlight,
    ...rawFlight,
    rawOption: rawFlight.rawOption || location.state?.fare?.rawOption,
  ...
  const buildPassengersPayload = () => {
    ...
    const selectedSeatsByPax = location.state?.selectedSeatsByPax || {};
    const mealObj = location.state?.addonsData?.mealObj || null;
    const baggageObjs = location.state?.addonsData?.addonObjs || [];
    const addonsByPax = location.state?.addonsData?.addonsByPax || {};
    ...
    return sourcePassengers.map((p, idx) => {
      // Per-passenger seat assignment
      const paxSeatObj = selectedSeatsByPax[idx]?.rawObj
        || (p.isLeadPax ? seatObj : null);
      const paxSeatDynamic = paxSeatObj ? [paxSeatObj] : [];

      // Per-passenger meal assignment
      const paxMealObj = addonsByPax[idx]?.mealObj
        || (p.isLeadPax ? mealObj : null);
      const paxMealDynamic = paxMealObj ? [paxMealObj] : [];

      // Per-passenger baggage assignment
      const paxBaggageObjs = addonsByPax[idx]?.addonObjs;
      const effectiveBaggage = (Array.isArray(paxBaggageObjs) && paxBaggageObjs.length > 0)
        ? paxBaggageObjs
        : (p.isLeadPax && baggageObjs.length > 0 ? baggageObjs : defaultBaggageEntries);
      ...
      return {
        ...
        // Fare object Adivaha requires per-passenger. In Adivaha/TBO, every
        // passenger in Passengers[] carries the authentic session Fare object
        // (including TaxBreakup, ChargeBU, PublishedFare, OfferedFare,
        // CommissionEarned, and TdsOnCommission). The provider matches PaxType
        // with this quote to price and issue individual passenger tickets.
        Fare: rawFare || flight.rawOption?.Fare,
        // Every passenger needs a real Baggage selection, not just the lead
        // pax: the purchased add-on if bought, otherwise the airline's own free/
        ...
        MealDynamic: paxMealDynamic,
        SeatDynamic: paxSeatDynamic
      };
    });
  };
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      const { data } = await axios.post(`${API_BASE_URL}/flights/validate-booking`, {
        ...
        sharedDetails: location.state?.sharedDetails || undefined,
      ...
      if (data?.revalidatedOptions?.[0]) {
        flight.rawOption = data.revalidatedOptions[0];
      }
    } catch (err) {
              ...
              <div className="space-y-5 text-sm font-bold text-[#1A1A1A] font-inter">
                ...
                <div className="flex justify-between items-start py-3 border-b border-[#EAEAEA]">
                  ...
                  <div className="text-right flex flex-col items-end">
                    ...
                    <span className="text-[12px] text-[#6B6B6B] font-medium mt-1">
                      {(() => {
                        const selectedSeatsByPax = location.state?.selectedSeatsByPax || {};
                        const seatList = Object.values(selectedSeatsByPax)
                          .map(seat => seat?.seatCode)
                          .filter(Boolean);
                        if (seatList.length > 0) {
                          return `Seats: ${seatList.join(", ")}`;
                        }
                        return location.state?.selectedSeat 
                          ? `Seat: ${location.state.selectedSeat}`
                          : "System assigned (free)";
                      })()}
                    </span>
                  </div>
                </div>
                ...
                {(() => {
                  ...
                  const seatList = Object.values(selectedSeatsByPax)
                    .map(seat => seat?.seatCode)
                    .filter(Boolean);
                  const singleSeat = location.state?.selectedSeat;
                  
                  if (seatList.length > 0) {
                    return (
                      <div className="flex justify-between items-start py-3 border-b border-[#EAEAEA]">
                        <span className="text-[#6B6B6B] font-bold text-[14px]">Seat{seatList.length > 1 ? "s" : ""}</span>
                        <div className="text-right flex flex-col items-end gap-1">
                          {seatList.map((seatCode, idx) => (
                            <span key={idx} className="text-[13px] text-[#1A1A1A] font-bold">
                              {seatCode}
                            </span>
                          ))}
                        </div>
                    ...
                    return (
                      <div className="flex justify-between items-start py-3 border-b border-[#EAEAEA]">
                        ...
                        <span className="text-[#1A1A1A] font-bold text-[14px]">{singleSeat}</span>
                      </div>
                    );
                  }
                  return null;
