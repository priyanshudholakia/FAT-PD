...
export default function BookingPage() {
  ...
  const [selectedSeatObj, setSelectedSeatObj] = useState(null); // raw SSR SeatDynamic entry for lead pax
  const [selectedSeatsByPax, setSelectedSeatsByPax] = useState({}); // per-passenger seats mapping
  ...
  const handleSeatSelect = (seatsByPaxOrCode, seatRawObjOrTotalPrice) => {
    if (typeof seatsByPaxOrCode === "object" && seatsByPaxOrCode !== null) {
      // Called with (selectedSeatsByPax, totalSeatPrice)
      setSelectedSeatsByPax(seatsByPaxOrCode);
      const leadSeat = seatsByPaxOrCode[0] || Object.values(seatsByPaxOrCode)[0] || null;
      setSelectedSeat(leadSeat?.seatCode || "");
      setSelectedSeatObj(leadSeat?.rawObj || null);
      if (typeof seatRawObjOrTotalPrice === "number") {
        setSeatPrice(seatRawObjOrTotalPrice);
      }
    } else {
      setSelectedSeat(seatsByPaxOrCode);
      setSelectedSeatObj(seatRawObjOrTotalPrice || null);
    }
  };
