...
export default function HeroSection() {
  ...
  const [departureDate, setDepartureDate] = useState(getOffsetDateStr(7)); // 7 days out (guaranteed active sandbox flights)
  const [returnDate, setReturnDate] = useState(getOffsetDateStr(14)); // 14 days out
  ...
  // Debounce timers to prevent rapid API calls
  const calendarDebounceRef = useRef({});

  // Fetch calendar fares for a specific month with debouncing
  const fetchCalendarFaresForMonth = async (dateObj, direction = "departure") => {
    if (!selectedFrom?.code || !selectedTo?.code) return;
    
    // Create debounce key
    const debounceKey = `${direction}-${selectedFrom.code}-${selectedTo.code}-${dateObj.toISOString()}`;
    
    // Clear existing timeout for this key
    if (calendarDebounceRef.current[debounceKey]) {
      clearTimeout(calendarDebounceRef.current[debounceKey]);
    }
    
    // Set new timeout
    calendarDebounceRef.current[debounceKey] = setTimeout(async () => {
      try {
  ...
  // DISABLED: fetchCalendarFareOfTheDay to reduce API usage and prevent 429 errors
  // This was making additional API calls for single-day updates which were causing rate limiting
  /*
