...
/**
 * Returns the authentic quoted Fare object that Adivaha's booking API expects
 * inside each Passengers[] entry.
 *
 * CRITICAL: In Adivaha/TBO, every passenger in Passengers[] must carry the
 * authentic session Fare object returned by FareQuote (including TaxBreakup,
 * ChargeBU, PublishedFare, OfferedFare, CommissionEarned, and TdsOnCommission).
 * The provider automatically prices and reconciles each passenger according to
 * their PaxType based on this quote. Fabricating a synthetic per-unit Fare
 * object (stripping TaxBreakup, ChargeBU, and forcing OfferedFare = PublishedFare)
 * causes Adivaha's ledger reconciliation to fail with Error 7606 ("Oops something
 * went wrong. Please contact support team").
 */
export function buildPerPassengerFare(rawOption, paxType) {
  return rawOption?.Fare || null;
}
