...
router.post("/search", async (req, res, next) => {
  try {
    ...
    try {
      ...
      console.warn("[flights/search] Provider search error:", error.message);
      res.status(200).json({
        success: false,
        message: "No flights found for this route or date. Please try another date.",
        responseData: { Response: { Results: [[]], Error: { ErrorCode: 1, ErrorMessage: "No flights found" } } }
      });
    }
  } catch (error) {
    next(error);
...
router.post("/calendar-fares", async (req, res, next) => {
  try {
    ...
    try {
      ...
      console.warn("[flights/calendar-fares] Provider calendar error:", error.message);
      res.status(200).json({
        success: false,
        responseData: { Response: { SearchResults: [] } }
      });
    }
  } catch (error) {
    next(error);
...
router.post("/update-calendar-fare", async (req, res, next) => {
  try {
    ...
    try {
      ...
      console.warn("[flights/update-calendar-fare] Provider update calendar error:", error.message);
      res.status(200).json({
        success: false,
        responseData: { Response: { SearchResults: [] } }
      });
...
router.post("/validate-booking", async (req, res, next) => {
  try {
    ...
    const { valid: passengersValid, missingFields } = validatePassengersAgainstRequirements(
      requirements,
      passengers,
      { gstin, gstCompanyName, couponCode, contact, sharedDetails: req.body?.sharedDetails }
    );
...
const runFinalRequirementsGate = async (providerPayload) => {
  try {
    ...
    // CRITICAL for multi-passenger bookings: In Adivaha/TBO, every passenger
    // in Passengers[] must carry the authentic session Fare object returned by
    // FareQuote (including TaxBreakup, ChargeBU, PublishedFare, OfferedFare,
    // CommissionEarned, and TdsOnCommission). The provider prices each passenger
    // according to PaxType based on this quote. If a passenger has a stripped-down,
    // fabricated, or stale per-passenger Fare, the provider rejects the booking
    // with Error 7606 ("Oops something went wrong. Please contact support team").
    // We re-bind the authentic freshly-quoted session Fare to every passenger here.
    if (opt?.Fare && Array.isArray(providerPayload?.Passengers)) {
      providerPayload.Passengers.forEach((p) => {
        p.Fare = opt.Fare;
      });
      console.log(`[Requirements Gate] Re-bound authentic quoted Fare for ${providerPayload.Passengers.length} passenger(s).`);
    }
