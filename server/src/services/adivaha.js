...
const executeRequest = async (requestFn, retries = 3, delay = 1000) => {
  ...
  for (let i = 0; i < retries; i++) {
    try {
      ...
      if (errorCode === 429) {
        const retryAfter = response?.data?.responseData?.Response?.Error?.RetryAfter || 5;
        const backoffTime = (retryAfter * 1000) + (Math.random() * 2000); // Add jitter
        console.warn(`[Adivaha] Rate limited (ErrorCode 429). Retrying after ${retryAfter}s with jitter (Attempt ${i + 1}/${retries})`);
      ...
      // Check for 429 error in catch block (might come as HTTP error)
      if (error.response?.data?.Error?.ErrorCode === 429 || error.response?.data?.responseData?.Response?.Error?.ErrorCode === 429) {
        const retryAfter = error.response?.data?.Error?.RetryAfter || error.response?.data?.responseData?.Response?.Error?.RetryAfter || 5;
        const backoffTime = (retryAfter * 1000) + (Math.random() * 2000);
        console.warn(`[Adivaha] Rate limited in catch block (ErrorCode 429). Retrying after ${retryAfter}s (Attempt ${i + 1}/${retries})`);
        await new Promise(resolve => setTimeout(resolve, backoffTime));
        continue;
      }
