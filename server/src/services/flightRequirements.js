/**
 * ============================================================================
 * PATH: server/src/services/flightRequirements.js
...
const isInternationalSegment = (seg) => {
  if (!seg.originCountry || !seg.destCountry) return null; // unknown
  return String(seg.originCountry).toUpperCase() !== String(seg.destCountry).toUpperCase();
};
...
export const computeFlightRequirements = (rawOptions) => {
  ...
  const hasInternationalSegment = internationalVotes.length > 0 ? internationalVotes.some(Boolean) : null; // null = unknown
  ...
  const passportFullDetailRequired = orFlag(options, "IsPassportFullDetailRequiredAtBook");
  // Explicit API flags always win. Only when NEITHER book nor ticket flags
  // are present on ANY option do we fall back to inferring from segments —
  // and even then, only when the segment data itself is conclusive.
  const passportFlagsExplicit = passportAtBookExplicit || passportAtTicketExplicit;
  const passportRequired = passportFlagsExplicit
  ...
  const itinerary = {
    passport: {
      ...
      requiredAtBook: passportRequiredAtBook,
      requiredAtTicket: passportRequiredAtTicket,
      fullDetailRequired: passportFullDetailRequired,
      source: passportFlagsExplicit ? "api_flag" : (hasInternationalSegment === null ? "unknown" : "inferred_from_segments")
    },
    pan: {
